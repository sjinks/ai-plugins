...
- **`trivially-safe`** — read-only inspection with no workspace, dependency, network, git, service, environment, or external state change, or a Local Workspace-Bounded Verification command that meets every condition in that section below.
