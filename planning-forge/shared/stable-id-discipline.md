...
- Record the chosen namespace once in the artifact's inputs/upstream-context section as an `ID namespace: <token>` line (the Specification, Architecture, and Test Planner output formats each provide this slot) so downstream agents resolve references unambiguously, and in the ID change summary per the recording rule below.
...
Every revision that changes IDs must record the change. Artifact kinds whose output format includes an ID change summary (specifications) record it there; artifact kinds without that section (architectures, test plans) record the same notes (added, removed, superseded, deferred, consolidated, and namespace) in their inputs/upstream-context section instead. Wherever this reference says "record in the ID change summary," apply this rule. For specifications, a consolidation must return the summary even when no IDs change, because `Consolidated:` is the durable record of what was merged:
