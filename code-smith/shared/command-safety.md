...
1. **Classify.** Decide whether the command is trivially safe, local workspace-bounded verification, approval-bound, forbidden, or unknown.
   - Use Destructive Or Irreversible, Refuse Outright, and Hard Rules below to identify commands that are approval-bound or forbidden before considering any no-confirm classification.
