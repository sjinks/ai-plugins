...
Follow the evidence and safety rules in the plugin's `shared/exploration-protocol.md` — in particular: use only already-installed tools. Output contracts: `08_DEPENDENCY_GRAPH.md` and `dependency_graph.json` in `shared/output-contracts.md`. Both files live at the plugin root, sibling of `skills/`. When run standalone, those rules still apply; if either reference is unavailable, stop and report it.
