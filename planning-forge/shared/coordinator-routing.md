...
Missing-artifact precondition: `architecture-request`, `test-plan-request`, and `publish-request` require their primary input to exist. `architecture-request` and `test-plan-request` require at least a specification (`test-plan-request` does not require architecture — it falls back to a spec-level plan); `publish-request` requires the artifact to be published. If the required input is missing, do not route on empty inputs; recommend creating it first (treat as `new-session` for a missing spec). Likewise, if `amend-spec` or `answer-open-questions` arrives with no base specification, reclassify as `new-session` and confirm with the user. `implementation-handoff-request` is governed by its own gate (below), which may proceed without architecture or a test plan when the user explicitly accepts those gaps; it still requires at least a specification (or an explicitly approved ready slice).
