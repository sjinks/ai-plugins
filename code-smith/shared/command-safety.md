...
## Trivially Safe: Local Workspace-Bounded Verification (run without confirmation)

Local build, test, lint, typecheck, static-analysis, or code-generation commands are classified `trivially-safe` (not a separate class) and may run without repeated confirmation when all of these are true:
