...
- Local build/test/lint/typecheck commands, including CMake configure/build/test commands, may run without repeated approval only after local evidence inspection and before/after workspace-state checks. Both agents classify such commands as `trivially-safe` under their shared Local Workspace-Bounded Verification criteria (`shared/command-safety.md` for Code Smith, `shared/verification-command-safety.md` for Test Smith); it is not a separate command class.
