...
| 10 | `10-advanced-invocation/` | "run the Architecture Planner for me now" (spec ready) | `architecture-request` | advanced mode: invoke one specialist, relay with Specialist Result Summary, stop; manual fallback if unavailable |
| 11 | `11-status-request/` | "where are we; give me a resumable summary" | `status-request` | no routing; report session state and carry-forward items with evidence-backed fields only |
