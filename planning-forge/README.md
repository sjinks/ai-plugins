...
- `shared/metamodel-export-mapping.md` — mapping from the metamodel to ReqIF and OSLC, both implemented as one-way exporters (import/round-trip remains a design note).
...
Machine-readable metamodel fixtures live under `dev/planning-forge/fixtures/metamodel/` (JSON and YAML, one per stage). Validate them with `node dev/planning-forge/scripts/validate-metamodel.mjs <artifact.(json|yaml|yml)>`. Generate human-readable views with `node dev/planning-forge/scripts/generate-metamodel-views.mjs <artifact> --view all`, report traceability completeness with `node dev/planning-forge/scripts/metamodel-completeness.mjs <artifact>`, export a one-way ReqIF 1.0 document with `node dev/planning-forge/scripts/export-metamodel-reqif.mjs <artifact> [--out <file>]`, export a one-way OSLC RDF/XML document with `node dev/planning-forge/scripts/export-metamodel-oslc.mjs <artifact> [--base <iri-prefix>] [--out <file>]`, and run regression checks with `node dev/planning-forge/scripts/test-metamodel-validator.mjs`, `node dev/planning-forge/scripts/test-metamodel-views.mjs`, `node dev/planning-forge/scripts/test-metamodel-reqif.mjs`, and `node dev/planning-forge/scripts/test-metamodel-oslc.mjs`.
