...
- Record the chosen namespace once in the artifact's inputs/upstream-context section as an `ID namespace: <token>` line (the Specification, Architecture, and Test Planner output formats each provide this slot) so downstream agents resolve references unambiguously. Also record it in the ID change summary when the artifact kind emits one (specifications).
